...
let params = {
  searchkw: '',
  categtopid: $.getStorage('categtopId'),
  catid: 0, // small catid
  bigcatid: 0,
  sort: 'new',
  fields: 0, // search type
  title_shop_categtop: '全部',
  title_shop_categ: '所有分类',
  title_shop_sort: '最新信息'
};
if (!params.categtopid) params.categtopid = 0;
...
let updateSubtabTitle = function() {
  let _params = $.getStorage('productListParams');
  $.byId('shop_categtop').title = _params.title_shop_categtop;
...
$.ready(function() {
  createIframe('categtop', {id: 'filterCategTop', url: 'filterCategTop' + viewEXT});
  ...
  // $.rmStorage('productListParams');
  if ($.getStorage('productListParams')) $.extend(params, $.getStorage('productListParams'));
  else $.setStorage('productListParams', params);

  updateSubtabTitle();
  // mescroll
  let downCallback = function() {
    let _params = $.getStorage('productListParams');
    delete _params.title_shop_categtop;
