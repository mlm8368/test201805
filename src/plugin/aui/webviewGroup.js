...
proto._initParent = function() {
	this.parent = window.plus.webview.getWebviewById(this.id);
	if(!this.parent) {
		this.parent = window.plus.webview.create(this.id, this.id);
...
proto._initWebviewContexts = function() {
	for(var len = this.items.length, i = len - 1; i >= 0; i--) {
		...
		if(isCurrent) {
		  webviewContext.showWaiting = false;
			webviewContext.webview = window.plus.webview.getWebviewById(id);
...
proto._initDrag = function(webview, dir) {
	...
	if(viewId) {
		var view = window.plus.webview.getWebviewById(viewId);
...
proto._checkDrags = function(webview) {
	...
	if(left) {
		var leftWebview = window.plus.webview.getWebviewById(left);
		if(leftWebview && !leftWebview.__mui_drag_left_flag) {
			this._initDrag(leftWebview, 'left');
		}
	}
	if(right) {
		var rightWebview = window.plus.webview.getWebviewById(right);
...
proto.switchTab = function(id) {
	...
	if(!toWebview) {
		...
		//					toWebview.showBehind(window.plus.webview.getSecondWebview());
		toWebview.show();
...
//	console.log("next:" + toWebview.id + ",from:" + toFromLeft);
	...
	window.plus.webview.startAnimation({
			'view': fromWebview,
			'styles': {
				'fromLeft': '0',
				'toLeft': fromToLeft
		...
		function(e) {
			...
			if(e.id === toWebview.id) {
				isNew && window.plus.nativeUI.showWaiting();
...
_proto.createWebview = function(from) {
	...
	if(this.webview) {
		...
		if(from !== 'startAnimation') {
			...
			if(this.showWaiting) window.plus.nativeUI.showWaiting();
		}
		this.webview = window.plus.webview.create(this.url, this.id, options.styles, options.extras);
		//append进去，避免返回时闪屏
		window.plus.webview.currentWebview().append(this.webview);
	}
	this._initWebview();
	this.inited = true;
};
